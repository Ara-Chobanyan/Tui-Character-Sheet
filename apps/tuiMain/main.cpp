...
// when the components are being rendered,
// we can have some background logic to see if there are any data for us to use.
#include <utility>
#include <array>
#include <atomic>
#include <chrono>
...
int main()
{
  ...
  auto htop{ Renderer([&] {
    auto frequency{ vbox({ text("Frequency [Mhz]") | hcenter,
      hbox(
        { vbox(
            { text("2400 "), filler(), text("1200 "), filler(), text("0 ") }),
          graph(std::ref(my_graph)) | flex })
        | flex }) };


    auto utilization{ vbox({ text("Utilization [%]") | hcenter,
      hbox({
        vbox({
          text("100 "),
          filler(),
    ...
    auto ram{ vbox({ text("Ram [Mo]") | hcenter,
      hbox(
  ...
  auto compiler_component{ Container::Horizontal({ compiler,
    flags,
    Container::Vertical({
      executable_,
  ...
  auto render_command{ [&] {
    ...
    if (!executable_content_.empty()) {
      // theory craft when something is not empty we can do something with that
      // data, while the tui is being rendered?
      line.push_back(text(" -o ") | bold),
        line.push_back(
          text(executable_content_) | color(Color::BlueLight) | bold);
    }
  ...
  auto compiler_renderer{ Renderer(compiler_component, [&] {
    auto compiler_win{ window(
      text("Compiler"), compiler->Render() | vscroll_indicator | frame) };

    auto flags_win{ window(
      text("Flags"), flags->Render() | vscroll_indicator | frame) };
  ...
  auto spinner_tab_renderer{ Renderer([&] {
    ...
    for (int i{ 0 }; i < 22; ++i) {
      entries.push_back(
        spinner(i, shift / 2) | bold | size(WIDTH, GREATER_THAN, 2) | bold);
    }
  ...
  auto color_tab_renderer{ Renderer([] {
...

    {
      ...
      palette_256_color_display =
        vbox({ palette_256_color_display, separator(), vbox(columns) })
        | border;
    }
...

    {
      ...
      true_color_display =
        vbox({ true_color_display, separator(), vbox(std::move(array)) })
        | border;
    }

    return flexbox(
      { basic_color_display, palette_256_color_display, true_color_display },
  ...
  auto gauge_compact{ Renderer([render_gauge] {
    return vbox({
  ...
  auto paragraph_renderer_left{ Renderer([&] {
    ...
    return vbox({ window(text("Align left:"), paragraphAlignLeft(str)),
             ...
             window(text("Side by side"),
               hbox({ paragraph(str), separator(), paragraph(str) })),
  ...
  auto paragraph_renderer_group_render{ Renderer(paragraph_renderer_group,
  ...
  auto tab_selection{ Menu(
    &tab_entries, &tab_index, MenuOption::HorizontalAnimated()) };
  ...
  auto tab_content{ Container::Tab(
    {
      ...
      gauge_compact,
      compiler_renderer,
      paragraph_renderer_group_render,
    },
    &tab_index) };
